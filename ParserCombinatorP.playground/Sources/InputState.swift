...
extension Position {
    ...
    /// This is line agnostic. It's upto the client to make sure
    /// that the line actually has this column
    func incrRow() -> Position {
        return Position(row: row + 1, col: 0)
    }
...
extension InputState {
    func parserPosition() -> ParserPosition {
        return ParserPosition(currentLine: self.currentLine, row: self.position.row, col: self.position.col)
    }
}
